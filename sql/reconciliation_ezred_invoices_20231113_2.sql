...
(
select distinct invoice_number from id_ezred.ds_1p_agreements where agreement_pct is null
),
...
from id_ezred.ds_1p_backupreport where bkup_invoice in (select invoice_number from dist_provisional_invoices)
and net_receipts >0
...
dist_agreements as 
(
select distinct agreement_number, agreement_title , agreement_pct, agreement_start_date , agreement_end_date from id_ezred.ds_1p_agreements
...
from id_ezred.ds_1p_backupreport b 
left join id_ezred.ds_1p_backupreport_revised br on b.asin=br.asin and b.purchase_order=br.purchase_order and b.bkup_agreement =br.bkup_agreement and b.transaction_type =br.transaction_type 
...
id_ezred.ds_1p_backupreport_revised where old_invoice_quantity =0 and transaction_type='Distributor Shipment'
),
...
br left join id_ezred.ds_1p_agreements 
...
and a.invoice_date>='2021-11-13' and a.invoice_date<='2023-10-31'
),
...
(
select * from id_ezred.ds_1p_invoices_details
--union all
...
(
select po, case when STRPOS(asin_received, asin )>0 then asin else asin_received end  asin, 
...
group by po, case when STRPOS(asin_received, asin )>0 then asin else asin_received end
),
