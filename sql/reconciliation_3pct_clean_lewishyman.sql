...
dist_agreements as 
(
select distinct agreement_number, agreement_title , agreement_pct, agreement_start_date , agreement_end_date from dc_raw.ds_astralbeauty_lewishyman_1p_agreements
...
from dc_raw.ds_astralbeauty_lewishyman_1p_backupreport b 
left join dc_raw.ds_astralbeauty_lewishyman_1p_backupreport_revised br on b.asin=br.asin and b.purchase_order=br.purchase_order and b.bkup_agreement =br.bkup_agreement and b.transaction_type =br.transaction_type 
...
dc_raw.ds_astralbeauty_lewishyman_1p_backupreport_revised where old_invoice_quantity =0 and transaction_type='Distributor Shipment'
),
...
--all_bkups as
--(
--select bkup_agreement, bkup_invoice, purchase_order, asin, transaction_type, quantity, net_receipts from dc_raw.ds_astralbeauty_lewishyman_1p_backupreport
--union all
...
br left join dc_raw.ds_astralbeauty_lewishyman_1p_agreements 
...
where a.agreement_number in (select distinct agreement_number from 
dist_agreements
--dc_raw.ds_astralbeauty_lhlicensed_1p_agreements
--dc_raw.ds_astralbeauty_lewishyman_1p_agreements
--where agreement_pct=10
) --'42506050' --and a.invoice_number ='6430-452738370123'
and a.backup_rpt_start_date>='2021-01-01' and a.backup_rpt_end_date<='2023-05-31'
),

--select count(*) from bkup_agreement
...
(
select * from dc_raw.ds_astralbeauty_lewishyman_1p_invoices_details i
...
select tb.agreement, tb.invoiced_fees, tb.total_net_receipts, orr.overbilling_by_invoice_received_qty, orr.underbilling_by_invoice_received_qty, orr.matched_by_invoice_received_qty,
...
--select tb.agreement, orr.overbilling_by_invoice_received_qty, urr.underbilling_by_invoice_received_qty, mr.matched_by_invoice_received_qty,
--oi.overbilling_by_invoice_qty, ui.underbilling_by_invoice_qty, mi.matched_by_invoice_qty,
--da.agreement_title, da.agreement_pct, da.agreement_start_date, da.agreement_end_date
--from total_bills tb left join overbill_totals_qty_received orr on tb.agreement=orr.agreement
--left join overbill_totals_qty_invoiced oi on tb.agreement=oi.agreement
--left join underbill_totals_qty_received urr on tb.agreement=urr.agreement
--left join matched_totals_qty_received mr on tb.agreement=mr.agreement
--left join underbill_totals_qty_invoiced ui on tb.agreement=ui.agreement
--left join matched_totals_qty_invoiced mi on tb.agreement=mi.agreement
--left join dist_agreements da on tb.agreement=da.agreement_number
...
--totals
select agreement, --ag.agreement_title , ag.agreement_pct, ag.agreement_start_date , ag.agreement_end_date , 
round(sum(overbilled_rebate_received),2) overbilled_rebate_received, round(sum(overbilled_rebate),2) overbilled_rebate --, round(sum(overbilled_rebate2),2) overbilled_rebate2 
, count(*) cnt
from all_data ad 
--left join dist_agreements ag on ad.agreement=ag.agreement_number 
where --excess_quantity_flag2=1
excess_quantity_flag_received=1
group by agreement
--, ag.agreement_title , ag.agreement_pct, ag.agreement_start_date , ag.agreement_end_date 
