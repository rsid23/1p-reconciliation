drop table dc_raw.ds_astralbeauty_lhlicensed_1p_agreements;
create table dc_raw.ds_astralbeauty_lhlicensed_1p_agreements
(
...
truncate table dc_raw.ds_astralbeauty_lhlicensed_1p_agreements;
COPY dc_raw.ds_astralbeauty_lhlicensed_1p_agreements (agreement_number, invoice_number, invoice_line_type, ccogs_invoice_id, agreement_title, funding_type, agreement_pct, agreement_start_date, agreement_end_date, agreement_raw_text, backup_rpt_file, backup_rpt_start_date, backup_rpt_end_date, cnt_backup_rptfiles) from 's3://iderive-ds-1p-reconciliation/astralbeauty/lhlicensed/agreements/' iam_role 'arn:aws:iam::467175486680:role/iderive-ds-reads3' format csv delimiter as ',' ignoreheader 1 BLANKSASNULL DATEFORMAT 'auto';
