...
(
select distinct invoice_number from ds_1p_agreements where agreement_pct is null
),
...
from ds_1p_backupreport where bkup_invoice in (select invoice_number from dist_provisional_invoices)
and net_receipts >0
...
dist_agreements as 
(
select distinct agreement_number, agreement_title , agreement_pct, agreement_start_date , agreement_end_date from ds_1p_agreements
...
case when br.new_rebate is not null then br.new_rebate else b.rebate_in_agreement_currency  end rebate_in_agreement_currency
...
from ds_1p_backupreport b 
left join ds_1p_backupreport_revised br on b.asin=br.asin and b.purchase_order=br.purchase_order and b.bkup_agreement =br.bkup_agreement and b.transaction_type =br.transaction_type 
...
(
select receive_date, bkup_agreement, bkup_invoice, purchase_order, asin, transaction_type, new_quantity, net_receipts, new_rebate rebate_in_agreement_currency from
ds_1p_backupreport_revised where old_invoice_quantity =0 and transaction_type='Distributor Shipment'
),

all_bkups as 
(
select receive_date, bkup_agreement, bkup_invoice, purchase_order, asin, transaction_type, quantity, net_receipts, rebate_in_agreement_currency from revised_bkups 
union
select receive_date, bkup_agreement, bkup_invoice, purchase_order, asin, transaction_type, new_quantity quantity, net_receipts, rebate_in_agreement_currency from non_zero
),
...
--and a.invoice_date>='2021-08-01' and a.invoice_date<='2023-08-31'
and a.invoice_date>='2021-11-01' and a.invoice_date<='2023-09-30'
),

bkup_agreement_totals as 
(
select bkup_agreement, sum(net_receipts) net_receipts, sum(rebate_in_agreement_currency) rebate_in_agreement_currency
from bkup_agreement
group by bkup_agreement
),

all_invoices_details as 
(
select * from ds_1p_invoices_details
--union all
...
pivot_amazon_fee as 
(
select bkup_agreement, agreement_pct, bkup_invoice, purchase_order, asin, --backup_rpt_start_date, backup_rpt_end_date, 
sum(quantity) amazon_fee_quantity, sum(net_receipts) amazon_fee_net_receipts 
from bkup_agreement  
where transaction_type='Distributor Shipment'
group by bkup_agreement, agreement_pct, bkup_invoice, purchase_order, asin --, backup_rpt_start_date, backup_rpt_end_date
),
...
sum(translate(qty, '$MXN, ', '')) vi_quantity, 
sum(translate(amount, '$MXN, ', '')) vi_amount,
...
sum(translate(case when quantity_received='' then '0' else quantity_received end, '$MXN, ', '')) vi_quantity_received, 
sum(translate(case when amount_received='' then '0' else amount_received end, '$MXN,' , '')) vi_amount_received,
...
all_data as 
(
select a.bkup_invoice, a.bkup_agreement agreement, a.purchase_order po, a.asin, a.amazon_fee_quantity amazon_billed_qty, 'MXN' as agreement_currency, --a.amazon_fee_net_receipts, 
v.vi_quantity vendor_invoice_qty, 'MXN' vendor_invoice_currency, 'MXN' currency_match, 1 as currency_conversion_factor, --v.vi_amount, 
...
--select * from pivot_amazon_fee where purchase_order='14NH28BN' and asin='B00KY0E8OK'
--select * from all_data where po in ('46YSWCTQ' ,'14NH28BN', '4E8EUOYY', '75F2NFFD', '81UR746U') and asin='B00KY0E8OK'




--select tb.agreement, tb.total_net_receipts, tb.invoiced_fees, 
-- orr.overbilling_by_invoice_received_qty,
--round( orr.overbilling_by_invoice_received_qty/tb.invoiced_fees,2)*100 oberbilling_pct,
-- --orr.underbilling_by_invoice_received_qty, orr.matched_by_invoice_received_qty,
...
bkup_report_file as 
(
select a.agreement||'_'|| a.po||'_'|| a.asin mykey,  a.agreement, a.bkup_invoice, a.po, a.asin, amazon_billed_qty, agreement_currency, 
...
--
--select agreement, sum(overbilled_rebate)
...
select agreement, round(sum(overbilled_rebate_received),2) overbilled_rebate --, round(sum(overbilled_rebate2),2) overbilled_rebate2 
from all_data ad
--left join 
--dist_agreements ag on ad.agreement=ag.agreement_number 
where --excess_quantity_flag2=1
excess_quantity_flag_received=1
group by agreement  order by --agreement_pct, 
agreement


bkup_agreement_totals
select trunc(sysdate)-365*2

select * from ds_1p_invoices_details dpid limit 100